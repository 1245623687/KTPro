...
typedef enum GX_AUTO_CONNECTION_STATUS_ENTRY
{
	GX_AUTO_CONNECTION_STATUS_OFF   = 0,              ///< 注册设备掉线重连回调后，设备掉线通知功能码
	GX_AUTO_CONNECTION_STATUS_ON    = 1,              ///< 注册设备掉线重连回调后，设备上电通知功能码
}GX_AUTO_CONNECTION_STATUS_ENTRY;
...
*/
//----------------------------------------------------------------------------------
typedef void (GX_STDC *GXDeviceAutoConnectCallBack) (GX_AUTO_CONNECTION_STATUS_ENTRY nConnectStatus, void *pUserParam);
...
/**
\brief      注册掉线事件回调函数
\attention  设备掉线事件，设备掉线或者上电时这些事件都可以通过这个接口的回调方式传出，
            用户不需要获取事件的时候调用GXUnregisterDeviceAutoConnectCallback接口注销回调函数
\param [in]hDevice     设备句柄
\param [in]pUserParam  用户私有数据
\param [in]callBackFun 用户注册的回调函数
\return    GX_STATUS_SUCCESS             操作成功，没有发生错误
...
*/
//----------------------------------------------------------------------------------
GX_API GXRegisterDeviceAutoConncetCallback(GX_DEV_HANDLE hDevice, 
										   void* pUserParam, 
										   GXDeviceAutoConnectCallBack callBackFun);
...
*/
//----------------------------------------------------------------------------------
GX_API GXUnregisterDeviceAutoConnectCallback(GX_DEV_HANDLE hDevice);
