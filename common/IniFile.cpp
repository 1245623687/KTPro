...
/*	测试代码
    ...
    string val;
    inifile.GetValue("SERVER","PORT",val);
    AfxMessageBox(val.c_str());
    inifile.SetValue("SERVER","PORT","test");
...
bool INIFile::ProcessLine(string strLine)
{
    ...
    else if (strLine.find("=") != 0)
	{
		...
		SetValue(m_FileContent.back().SectionName,strLine.substr(0,ePos),strLine.substr(ePos+1,strLine.size()),false);
    }
    else
	{
		return false;
...
bool INIFile::SetValue(string strSection, string varName, string varvalue, bool breplace)
{
	//先查找段
...
bool INIFile::GetValue(string strSection, string varName, string &varvalue)
{
	//先查找段
	itVector it = m_FileContent.begin();
	for (it;it!=m_FileContent.end();it++)
