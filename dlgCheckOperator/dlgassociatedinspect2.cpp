#include "dlgassociatedinspect2.h"
#include "ui_dlgassociatedinspect2.h"
...
DlgAssociatedInspect2::DlgAssociatedInspect2(CheckOperatorAssociatedInspect2* pCheckOperator,ImgPro* imgPro,OpencvImage&  image,int basis_x,int basis_y,int mode,QWidget *parent) :
    m_CheckOperator(pCheckOperator),
    m_ImgPro(imgPro),
    m_Mode(mode),
    QDialog(parent),
    ui(new Ui::DlgAssociatedInspect2)
{
    ui->setupUi(this);
    this->setWindowTitle("关联检查");
    InitStyle();
    ui->lab_Title->setText(QString("关联检查--C%1").arg(pCheckOperator->stu_CheckOperatorBasic->Index));
    ...
    checkboxDelegate=new QCheckBoxDelegate(NULL);
...
void DlgAssociatedInspect2::showKeyBoard(KControlsBase* pLineEdit)
{
...
void DlgAssociatedInspect2::showPixelInfo(int x, int y)
{
...
void DlgAssociatedInspect2::InitStyle()
{
...
bool DlgAssociatedInspect2::eventFilter(QObject *obj, QEvent *event)
{
...
void DlgAssociatedInspect2::mouseMoveEvent(QMouseEvent *e)
{
...
void DlgAssociatedInspect2::mousePressEvent(QMouseEvent *e)
{
...
void DlgAssociatedInspect2::mouseReleaseEvent(QMouseEvent *)
{
    mousePressed = false;
}

void DlgAssociatedInspect2::on_btnMenu_Close_clicked()
{
    this->close();
}

void DlgAssociatedInspect2::updateCheckRes()
{
    //this->m_Scene->clear();
    int hei=m_ItemImage->getHeight();
    int wid=m_ItemImage->getWidth();
    CheckOperatorAssociatedInspect2Control * pCheckOperatorAssociatedInspect2Control=new CheckOperatorAssociatedInspect2Control(this->m_CheckOperator);
    ...
    bool res=pCheckOperatorAssociatedInspect2Control->debugCalculator(this->m_ItemImage->getImage().ImageGray,this->m_ItemImage->getImage().ImageGray,imgRes.ImageGray,
    ...
    delete  pCheckOperatorAssociatedInspect2Control;
}


QString DlgAssociatedInspect2::getCheckOperatorName(ENUMCHECKOPERATORTYPE t)
{
    QString ret="";
    switch (t)
    {
...
void DlgAssociatedInspect2::loadHVPos()
{
    ...
    ui->tableViewV->setColumnWidth(0,100);
    ui->tableViewV->setColumnWidth(1,100);
    ui->tableViewV->setColumnWidth(2,162);
    ui->tableViewV->setColumnWidth(3,162);


    ui->tableViewV->setItemDelegateForColumn(3, checkboxDelegate);
    ...
    ui->tableViewV->setSelectionBehavior(QAbstractItemView::SelectRows);
    ui->tableViewV->setVerticalScrollBarPolicy(Qt::ScrollBarAlwaysOn);
    ui->tableViewV->verticalHeader()->hide();
    ui->tableViewV->verticalHeader()->setDefaultSectionSize(36);

    {
        list<CheckOperatorBasic*>::iterator itor=m_ImgPro->LstCheckOperator.begin();
        int row=0;
        for(;itor!=m_ImgPro->LstCheckOperator.end();itor++,row++)
        {
            ...
            this->m_ModelVPos->setItem(row,0,itemIndex);
            this->m_ModelVPos->setItem(row,1,itemNo);
            this->m_ModelVPos->setItem(row,2,itemName);

            ui->tableViewV->model()->index(row,3);
            ui->tableViewV->model()->setData( ui->tableViewV->model()->index(row,3),(*itor)->stu_CheckOperatorBasic->IsAssociated,Qt::UserRole);

            checkboxDelegate->mapValue[row]=(*itor)->stu_CheckOperatorBasic->IsAssociated;
           //this->m_ModelVPos->setItem(row,3,itemState);
        }
    }
...
void DlgAssociatedInspect2::initControl()
{
...
DlgAssociatedInspect2::~DlgAssociatedInspect2()
{
...
void DlgAssociatedInspect2::on_pushBtnAutoCalc_clicked()
{
    if(m_Res==-1)
    {
...
void DlgAssociatedInspect2::saveParam()
{
   //  checkboxDelegate->mapValue[row]=(*itor)->stu_CheckOperatorBasic->IsAssociated;
    {
        list<CheckOperatorBasic*>::iterator itor=m_ImgPro->LstCheckOperator.begin();
        ...
        for(;itor!=m_ImgPro->LstCheckOperator.end();itor++,row++)
        {
           (*itor)->stu_CheckOperatorBasic->IsAssociated= checkboxDelegate->mapValue[row];
        }
    }
...
void DlgAssociatedInspect2::on_pushBtnSaveReturn_clicked()
{
    //updateCheckRes();
    saveParam();
    this->close();
...
void DlgAssociatedInspect2::on_pushBtnCancel_clicked()
{
    this->close();
}

//void DlgAssociatedInspect2::on_tableViewH_doubleClicked(const QModelIndex &index)
//{
...
//void DlgAssociatedInspect2::on_tableViewV_doubleClicked(const QModelIndex &index)
//{
...
void DlgAssociatedInspect2::on_toolButtonMin_clicked()
{
    this->ui->graphicsView->scaleMin();
    this->ui->graphicsView_2->scaleMin();
}

void DlgAssociatedInspect2::on_toolButtonMax_clicked()
{
...
void DlgAssociatedInspect2::on_toolButtonInit_clicked()
{
...
void DlgAssociatedInspect2::on_spinBoxBinThresh_valueChanged(int arg1)
{
    updateCheckRes();
}

void DlgAssociatedInspect2::on_spinBoxMinArea_valueChanged(int arg1)
{
    updateCheckRes();
}

void DlgAssociatedInspect2::on_radioBtnIsBlackBlob_clicked()
{
    updateCheckRes();
}

void DlgAssociatedInspect2::on_radioBtnIsWhiteBlob_clicked()
{
    updateCheckRes();
}

void DlgAssociatedInspect2::on_radioBtnDefectAll_toggled(bool checked)
{
    updateCheckRes();
}
...
void QCheckBoxDelegate::paint(QPainter *painter, const QStyleOptionViewItem &option, const QModelIndex &index) const
{
...
bool QCheckBoxDelegate::editorEvent(QEvent *event, QAbstractItemModel *model, const QStyleOptionViewItem &option, const QModelIndex &index)
{
    ...
    if (event->type() == QEvent::MouseButtonPress && decorationRect.contains(mouseEvent->pos()))
    {
        if (index.column() == 3)
        {
            ...
            bool ret=model->data(index,Qt::UserRole).toBool();
            mapValue[index.row()]=ret;
